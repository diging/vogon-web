...
def update_relation(template_part, provided, relationset, cache={}, appellation_cache={}, project_id=None, creator=None, text=None, required=None, _as_key=None):
    ...
    for pred in ['source', 'predicate', 'object']:    # Collect field data
        ...
        if datum:
            ...
            relation_data[dkey] = update_relation(getattr(template_part, '%s_relationtemplate' % pred), provided, relationset, cache=cache, appellation_cache=appellation_cache, project_id=project_id)
        else:
            payload = {
                'type': node_type,
