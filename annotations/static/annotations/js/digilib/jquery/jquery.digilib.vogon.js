...
(function($) {
    ...
    var defaults = {
        ...
        'onNewRegion' : function(data, $regionDiv) {
            $('body').trigger('regionSelected', [data, $regionDiv, packCoords($regionDiv.data().rect)]);
        },
    ...
    var actions = {
        defineUserRegion : function (data) {
            var onComplete = function (data, rect) {
                ...
                fn.highlightButtons(data, 'defineUserRegion', 0);
                redisplay(data);
                $(data).trigger('newRegion', [$regionDiv]);
            };
            fn.highlightButtons(data, 'defineUserRegion', 1);
    ...
    var installButtons = function(data) {
        ...
        var buttonSet = settings.vogonSet;

        console.log('buttonSet', buttonSet)
        buttonSettings.vogonSet = buttonSet;
        buttonSettings.buttonSets.push('vogonSet');
        buttonSettings.standardSet.push('defineUserRegion');
    ...
    var install = function(plugin) {
        digilib = plugin;
        console.debug('installing vogonWeb plugin. digilib:', digilib);
    ...
    var init = function (data) {
        console.debug('initialising vogon plugin. data:', data);
        var $data = $(data);
        var $elem = data.$elem;
        var settings = data.settings;
    ...
    if ($.fn.digilib == null) {
        $.error("jquery.digilib.vogon.js must be loaded after jquery.digilib!");
    } else {
