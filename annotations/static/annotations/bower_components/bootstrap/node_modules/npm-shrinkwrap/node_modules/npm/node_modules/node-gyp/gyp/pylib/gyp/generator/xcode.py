...
class XcodeProject(object):
  ...
  def Finalize1(self, xcode_targets, serialize_all_tests):
    ...
    for xck, xcv in list(self.build_file_dict.get('xcode_settings', {}).items()):
    ...
    if build_file_configurations:
      for config_name in configurations:
        ...
        if build_file_configuration_named:
          ...
          for xck, xcv in list(build_file_configuration_named.get('xcode_settings',
                                                             {}).items()):
        ...
        if command.get('environment'):
          script = script + "\n".join(
            ['export %s="%s"' %
             ...
             for (key, val) in list(command.get('environment').items())]) + "\n"
...
def PerformBuild(data, configurations, params):
  ...
  for build_file, build_file_dict in list(data.items()):
...
""" % makefile_name
    ...
    for configuration_name in configuration_names:
      ...
      if 'xcode_settings' in configuration:
        for xck, xcv in list(configuration['xcode_settings'].items()):
